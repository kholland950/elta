...
const messageHandlers: Record<string, Function> = {
    [MessageEventType.NEW_PLAYER]: (data: NewPlayerMessage, scene: MainScene) => {
        scene.playerManager.addPlayer(data.color, data.name, false, 0, 0, data.id)
    },
...
export class MessageBroker {
    ...
    constructor(scene: MainScene) {
        ...
        if (this.socket) {
            this.socket.onopen = () => {
                this.messageQueue.forEach(message => this.send(message.event, message.data as SendableMessageData))
                this.messageQueue = []
                this.socket.onmessage = (event: MessageEvent) => { this.receiveMessage(event) }
            }
        } else {
    ...
    schedule(event: MessageEventType, callback: (...args: any[]) => SendableMessageData | undefined, ...callbackArgs: any[]) {
